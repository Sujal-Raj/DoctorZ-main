...
export const createReferral = async (req, res) => {
    try {
        const { type, patientId, referredToHospitalId, referredToDoctorId, referredToLabId, reason, notes, priority, attachments } = req.body;
        ...
        if (referredToHospitalId) {
            if (isObjectId(referredToHospitalId)) {
                ...
                if (foundClinic)
                    referralData.referredToHospitalId = foundClinic._id;
                else
                    referralData.externalHospitalName = referredToHospitalId;
            }
        }
        if (referredToDoctorId)
            referralData.referredToDoctorId = referredToDoctorId;
        if (referredToLabId) {
            if (isObjectId(referredToLabId)) {
                ...
                if (foundLab)
                    referralData.referredToLabId = foundLab._id;
                else
                    referralData.externalLabName = referredToLabId;
            }
        }
...
export const addReferralMessage = async (req, res) => {
    try {
        const { id } = req.params;
...
export const uploadReport = async (req, res) => {
    try {
        ...
        let attachmentUrl = "";
        if (file) {
            attachmentUrl = await uploadToCloudinary(file.buffer, "referral_reports", file.mimetype === "application/pdf" ? "raw" : "image");
        }
        if (attachmentUrl) {
        ...
        if (message) {
            referral.thread.push({
                ...
                senderName: req.user.role === "doctor" ? "Doctor" : req.user.role === "lab" ? "Lab" : "Hospital",
                message,
                createdAt: new Date()
            });
        }
