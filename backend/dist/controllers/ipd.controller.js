...
export const getWardsList = async (req, res) => {
    try {
...
export const admitPatient = async (req, res) => {
    try {
        const { clinicId, patientId, doctorId, wardId, bedId, reasonForAdmission, emergencyContactName, emergencyContactRelation, emergencyContactPhone, initialDeposit, } = req.body;
...
export const addNursingNote = async (req, res) => {
    try {
        const { admissionId } = req.params;
        const { note, recordedBy } = req.body;
        const admission = await ipdAdmissionModel.findById(admissionId);
        if (!admission)
            return res.status(404).json({ success: false, message: "Admission log not found" });
...
export const addMARRecord = async (req, res) => {
    try {
        const { admissionId } = req.params;
