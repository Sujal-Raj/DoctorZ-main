...
export const adminLogin = async (req: Request, res: Response) => {
  try {
    ...
    const token = jwt.sign(
      { id: admin._id, adminId: admin.adminId, role: admin.role, isSuperAdmin: admin.role === "super_admin" },
...
export const getDashboardStats = async (req: Request, res: Response) => {
  try {
    ...
    paidLabPkgs.forEach((b: any) => monthlyRevenue += (b.packageId?.totalPrice || 0));
...
export const updateHospitalStatus = async (req: Request, res: Response) => {
  try {
    ...
    const hospital = await clinicModel.findByIdAndUpdate(
      hospitalId,
      { status },
      { new: true }
    );
