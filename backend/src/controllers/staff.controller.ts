...
export const staffLogin = async (req: Request, res: Response) => {
  try {
    ...
    const token = jwt.sign(
      { id: staff._id, clinicId: staff.clinicId, role: staff.role, permissions: staff.permissions },
...
export const getLeavesList = async (req: Request, res: Response) => {
  try {
    const { clinicId } = req.params;
    ...
    const flatLeaves = staffWithLeaves.flatMap(s => 
      s.leaves ? s.leaves.map(l => ({
        staffId: s._id,
        fullName: s.fullName,
        role: s.role,
        leaveId: (l as any)._id,
        date: l.date,
        reason: l.reason,
        status: l.status
      })) : []
...
export const approveRejectLeave = async (req: Request, res: Response) => {
  try {
    ...
    const leave = staff.leaves.find(l => String((l as any)._id) === leaveId);
    if (!leave) {
